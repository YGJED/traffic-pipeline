...
def reset_output_dirs():
    for path in [HIST_OUTPUT_DIR, STREAM_OUTPUT_DIR]:
        # These output dirs are bind-mounted volumes in docker-compose. Deleting the mount
        # point itself fails with "Device or resource busy", so we clear contents instead.
